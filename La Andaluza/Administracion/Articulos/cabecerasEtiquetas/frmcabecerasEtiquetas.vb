Imports BasesParaCompatibilidad.DataGridViewExtension
Public Class frmcabecerasEtiquetas
    Inherits BasesParaCompatibilidad.gridsimpleform


    Private dbocabecerasEtiquetas As DBO_cabecerasEtiquetas

    Public Sub New(Optional ByVal MaestroID As Integer = 0)
        MyBase.New(New spcabecerasEtiquetas, MaestroID)
        InitializeComponent()
        dbocabecerasEtiquetas = New DBO_cabecerasEtiquetas

        MyBase.newRegForm = CType(New frmEntcabecerasEtiquetas(BasesParaCompatibilidad.gridsimpleform.ACCION_INSERTAR, sp), BasesParaCompatibilidad.DetailedSimpleForm)
    End Sub


    Private Sub Insert_Before() Handles MyBase.BeforeInsert
        newRegForm.SetDataBussinesObject(CType(Me.dbocabecerasEtiquetas, BasesParaCompatibilidad.DataBussines))
    End Sub

    Private Sub modify_Before() Handles MyBase.BeforeModify
        Dim dtb As New BasesParaCompatibilidad.DataBase
        dbocabecerasEtiquetas = CType(sp, spcabecerasEtiquetas).Select_Record(CType(dgvGeneral.CurrentRow.Cells("Id").Value, Integer), dtb)
        If Not dbocabecerasEtiquetas Is Nothing Then
            newRegForm.SetDataBussinesObject(CType(Me.dbocabecerasEtiquetas, BasesParaCompatibilidad.DataBussines))
        Else
            MyBase.EventHandeld = True
            MessageBox.Show("No se pudo recuperar los datos", "Atención", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        End If
    End Sub

    Protected Overrides Sub BindDataSource()

        If Not dataSource Is Nothing Then
            GeneralBindingSource.DataSource = dataSource
            With dgvGeneral
                .DataSource = GeneralBindingSource
                .Columns("Id").Visible = False
                .FormatoColumna("texto", BasesParaCompatibilidad.TiposColumna.Izquierda, True)

            End With
        End If

    End Sub

End Class
