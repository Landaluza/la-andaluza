...
SELECT
     ...
     ,[dbo].[Proveedores].Nombre ProveedorID
     ,[dbo].[Articulos1].DescripcionLA ArticuloID
...
FROM
     [dbo].[AvisosPedidos]
     left JOIN [dbo].[Proveedores] ON [dbo].[AvisosPedidos].[Id_proveedor] = [dbo].[Proveedores].[ProveedorID]
     left JOIN [dbo].[Articulos1] ON [dbo].[AvisosPedidos].[Id_articulo] = [dbo].[Articulos1].[ArticuloID]
End

GO
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AvisosPedidosSelectDgvBy]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[AvisosPedidosSelectDgvBy]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[AvisosPedidosSelectDgvBy] @ProveedorID int, @ArticuloID int
AS 
BEGIN
SELECT
      [dbo].[AvisosPedidos].[Id] Id
...
FROM
     ...
     left JOIN [dbo].[Articulos1] ON [dbo].[AvisosPedidos].[Id_articulo] = [dbo].[Articulos1].[ArticuloID]
where
	articuloid = @ArticuloID or ProveedorID = @ProveedorID
End
